...
const CervicalCancerForm = ({ onClose }) => {
  // State management
  ...
  const validateCurrentPage = () => {
    switch (currentPage) {
      case 1:
        return formData.age && formData.numPartners && formData.firstIntercourseAge && formData.numPregnancies;
      case 2:
        return true;
      case 3:
        return true;
  ...
  const calculateRiskPrediction = () => {
    ...
    const firstIntercourse = parseInt(formData.firstIntercourseAge);
    
    if (age > 30) {
      riskScore += 1;
      riskFactors.push("Age over 30");
    }
    
    if (firstIntercourse && firstIntercourse < 18) {
      riskScore += 2;
      riskFactors.push("Early sexual activity (under 18)");
    }
    
    // Sexual partners
    const partners = parseInt(formData.numPartners);
    if (partners > 1) riskScore += 1;
    if (partners > 3) {
      riskScore += 2;
      riskFactors.push("Multiple sexual partners (>3)");
    }
    
    // Pregnancies
    const pregnancies = parseInt(formData.numPregnancies);
    if (pregnancies > 3) {
      riskScore += 1;
      riskFactors.push("Multiple pregnancies (>3)");
    }
    ...
    if (formData.smoking === "Yes") {
      ...
      const smokingYears = parseInt(formData.yearsSmoking);
      if (smokingYears > 10) {
        riskScore += 2;
        riskFactors.push("Long-term smoking (>10 years)");
      }
    }
    
    // Contraceptives
    if (formData.contraceptives === "Yes") {
      const contraceptiveYears = parseInt(formData.yearsContraceptives);
      if (contraceptiveYears > 5) {
        riskScore += 1;
        riskFactors.push("Long-term hormonal contraceptive use");
      }
    }
    
    // STDs
    if (formData.stds === "Yes") {
      riskScore += 2;
      riskFactors.push("History of STDs");
    }
    
    // HPV
    if (formData.stdHPV === "Yes") {
      riskScore += 5;
      riskFactors.push("HPV infection (major risk factor)");
    }
    
    // HIV
    if (formData.stdHIV === "Yes") {
      riskScore += 3;
      riskFactors.push("HIV infection (compromised immunity)");
    }
    
    let riskLevel, riskColor, recommendations;
    let riskPercentage = Math.min(95, Math.max(5, (riskScore * 4) + 10));
    ...
    if (riskScore <= 3) {
      ...
      riskPercentage = Math.min(30, riskPercentage);
    } else if (riskScore <= 7) {
      riskLevel = "Moderate";
      riskColor = "yellow";
      recommendations = [
        "Discuss more frequent screening with your healthcare provider",
        "Consider annual Pap smears and HPV testing",
        "Lifestyle modifications (quit smoking if applicable)",
        "Regular gynecological check-ups"
      ];
      riskPercentage = Math.min(65, Math.max(31, riskPercentage));
    } else {
      riskLevel = "Higher";
      riskColor = "red";
      recommendations = [
        "Consult healthcare provider immediately for personalized screening plan",
        "May need more frequent screening (every 6-12 months)",
      ...
      riskPercentage = Math.max(66, riskPercentage);
    }
      ...
      <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-3xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
          ...
          <div className="bg-gradient-to-r from-rose-500 via-pink-500 to-red-500 text-white p-6 sm:p-8 rounded-t-3xl relative overflow-hidden">
            ...
            <p className="text-rose-100 text-base sm:text-lg">Your personalized cervical cancer risk analysis is ready.</p>
          </div>
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-amber-700 text-sm leading-relaxed">
                    ...
                    Please consult with your healthcare provider for proper screening and diagnosis.
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="sticky top-0 bg-white/95 backdrop-blur-md border-b-2 border-rose-200 p-4 sm:p-6 z-10">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Cervical Cancer Risk Assessment</h1>
                <p className="text-gray-600 text-sm sm:text-base font-medium">Comprehensive reproductive health screening tool</p>
              </div>
            </div>
            <button
              onClick={onClose}
            ...
            <div className="space-y-8">
              ...
              {currentPage === 1 && (
                <div className="space-y-8 animate-in slide-in-from-right duration-500">
                  <FormSection title="Personal & Sexual Health" icon={User} gradient="from-rose-500 to-pink-600">
                    <FormField
                      label="Age"
                      name="age"
                      type="number"
                    ...
                    <FormField
                      ...
                      description="Total lifetime sexual partners"
                    />
                    <FormField
                      label="Age at First Intercourse"
                      name="firstIntercourseAge"
                      type="number"
                      value={formData.firstIntercourseAge}
                      onChange={handleChange}
                      required
                      placeholder="Enter age"
                      description="Age when you first became sexually active"
                    />
                    <FormField
                      ...
                      required
                      placeholder="Enter number"
                      description="Total pregnancies (including miscarriages)"
                    />
                  </FormSection>
                </div>
              )}
              ...
              {currentPage === 2 && (
                <div className="space-y-8 animate-in slide-in-from-right duration-500">
                  <FormSection title="Lifestyle & Medical History" icon={Heart} gradient="from-pink-500 to-rose-600">
                    <FormField
                      label="Do you smoke?"
                      name="smoking"
                    ...
                    <FormField
                      ...
                      onChange={handleChange}
                      placeholder="Enter years (if applicable)"
                      description="Total years you have smoked"
                    />
                    <FormField
                      ...
                      description="Birth control pills or other hormonal contraceptives"
                    />
                    <FormField
                      label="Years on Contraceptives"
                      name="yearsContraceptives"
                      type="number"
                      value={formData.yearsContraceptives}
                      onChange={handleChange}
                      placeholder="Enter years (if applicable)"
                      description="Total years using hormonal contraceptives"
                    />
                  </FormSection>
                </div>
              )}
              ...
              {currentPage === 3 && (
                <div className="space-y-8 animate-in slide-in-from-right duration-500">
                  <FormSection title="STD History & Additional Factors" icon={Shield} gradient="from-rose-500 to-pink-600">
                    <FormField
                      ...
                      description="Ever diagnosed with sexually transmitted diseases"
                    />
                    <FormField
                      label="Number of STDs"
                      name="numSTDs"
                      type="number"
                      value={formData.numSTDs}
                      onChange={handleChange}
                      placeholder="Enter number (if applicable)"
                      description="Total number of different STDs diagnosed"
                    />
                    <div className="lg:col-span-2 space-y-6">
                      <FormField
                        ...
                        description="HPV diagnosis (major risk factor for cervical cancer)"
                      />
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <FormField
                          label="HIV Status"
                          name="stdHIV"
                          value={formData.stdHIV}
                          onChange={handleChange}
                          options={[
                            { value: "No", label: "No" },
                            { value: "Yes", label: "Yes" }
                          ]}
                          description="HIV diagnosis status"
                        />
                        <FormField
                          label="IUD Usage"
                          name="iud"
                          ...
                          description="Intrauterine device usage"
                        />
                      </div>
                    </div>
                  </FormSection>
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl p-8 shadow-2xl max-w-xl text-center border-2 border-rose-200">
        ...
        <h1 className="text-3xl font-bold text-gray-900 mb-4">
          Welcome to the Cervical Cancer Risk Assessment
        </h1>
        <p className="text-gray-600 text-lg mb-8">
          This tool helps evaluate your potential risk factors for cervical cancer based on medical history and lifestyle factors.
        </p>
        <button

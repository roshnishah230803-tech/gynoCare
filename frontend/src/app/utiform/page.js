...
const UTIForm = ({ onClose }) => {
  // State management
  const [formData, setFormData] = useState({
    age: "",
    sexualActivity: false,
    burningUrination: "None",
    frequency: "",
    urgency: false,
    cloudyUrine: false,
    bloodUrine: false,
  ...
  const handleChange = (e) => {
    ...
    if (type === "checkbox") {
      if (name === "comorbidities") {
        let updatedComorbidities;
        if (checked) {
          updatedComorbidities = [...formData.comorbidities, value];
        } else {
          updatedComorbidities = formData.comorbidities.filter((item) => item !== value);
        }
        setFormData((prev) => ({ ...prev, comorbidities: updatedComorbidities }));
      } else {
  ...
  const validateCurrentPage = () => {
    switch (currentPage) {
      case 1:
        return formData.age !== "" && formData.burningUrination !== "";
      case 2:
        return formData.pelvicPain !== "";
      case 3:
        return true;
      default:
        return true;
  ...
  const calculateRiskPrediction = () => {
    ...
    if (formData.sexualActivity) {
      riskScore += 2;
      riskFactors.push("Sexual activity");
    }

    const severityMap = { "None": 0, "Mild": 2, "Moderate": 4, "Severe": 6 };
    riskScore += severityMap[formData.burningUrination];
    if (formData.burningUrination !== "None") {
      riskFactors.push(`Burning urination (${formData.burningUrination})`);
    }

    if (formData.frequency > 8) {
      riskScore += 3;
      riskFactors.push(`High urination frequency (${formData.frequency} times/day)`);
    }
    if (formData.urgency) {
      riskScore += 2;
      riskFactors.push("Urgency to urinate with little urine");
    }
    if (formData.cloudyUrine) {
      riskScore += 2;
      riskFactors.push("Cloudy or strong-smelling urine");
    }
    if (formData.bloodUrine) {
      riskScore += 5;
      riskFactors.push("Blood in urine (hematuria)");
    }

    riskScore += severityMap[formData.pelvicPain];
    if (formData.pelvicPain !== "None") {
      riskFactors.push(`Pelvic pain (${formData.pelvicPain})`);
    }

    if (formData.fever) {
      riskScore += 6;
      riskFactors.push("Fever or chills");
    }
    if (formData.nausea) {
      riskScore += 4;
      riskFactors.push("Nausea or vomiting");
    }

    if (formData.recurrentUTI) {
      riskScore += 5;
      riskFactors.push("History of recurrent UTIs");
    }
    formData.comorbidities.forEach(condition => {
      riskScore += 4;
      riskFactors.push(`Underlying condition (${condition})`);
    });
    ...
    if (riskScore <= 12) {
      ...
      riskColor = "yellow";
      recommendations = [
        "Increase your fluid intake to help flush your urinary tract.",
        "Consider discussing your symptoms with a pharmacist or doctor.",
        "Avoid self-treating with unverified remedies.",
      ];
      ...
      riskColor = "red";
      recommendations = [
        "It is strongly recommended you consult a healthcare professional immediately.",
        "A proper diagnosis and timely treatment are crucial to prevent a kidney infection.",
      ...
      <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-3xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
          ...
          <div className="bg-gradient-to-r from-rose-500 via-pink-500 to-red-500 text-white p-6 sm:p-8 rounded-t-3xl relative overflow-hidden">
            ...
            <p className="text-rose-100 text-base sm:text-lg">Your personalized UTI risk analysis is ready.</p>
          </div>
            ...
            <div className={`p-6 sm:p-8 rounded-3xl border-2 relative overflow-hidden ${
              ...
              {riskPrediction.color === 'red' && (
                <div className="flex items-center space-x-3 p-4 bg-red-100 rounded-2xl border border-red-200">
                  ...
                  <p className="text-red-800 font-semibold">High risk detected - Immediate medical consultation recommended</p>
                </div>
              )}
            </div>
        ...
        <div className="sticky top-0 bg-white/95 backdrop-blur-md border-b-2 border-rose-200 p-4 sm:p-6 z-10">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-xl sm:text-2xl font-bold text-gray-900">UTI Risk Assessment</h1>
                <p className="text-gray-600 text-sm sm:text-base font-medium">Comprehensive health evaluation tool</p>
              </div>
            </div>
            <button
              onClick={onClose}
            ...
            <div className="space-y-8">
              ...
              {currentPage === 1 && (
                <div className="space-y-8 animate-in slide-in-from-right duration-500">
                  <FormSection title="General & Primary Symptoms" icon={User} gradient="from-rose-500 to-pink-600">
                    <FormField
                      label="Age"
                      name="age"
                      type="number"
                    ...
                    <FormField
                      ...
                      onChange={handleChange}
                      description="Sexual activity can increase UTI risk"
                    />
                    <div className="lg:col-span-2 space-y-6">
                      <FormField
                        label="Burning/Painful Urination"
                        name="burningUrination"
                        type="radio"
                        value={formData.burningUrination}
                        onChange={handleChange}
                        radioOptions={[
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <FormField
                          ...
                          onChange={handleChange}
                          description="Strong, persistent urge to urinate"
                        />
                        <FormField
                          label="Urination Frequency (24 hrs)"
                          name="frequency"
                          type="number"
                          value={formData.frequency}
                          onChange={handleChange}
                          placeholder="e.g., 10"
                          description="Times you urinate per day"
                        />
                      </div>
                    </div>
                  </FormSection>
              ...
              {currentPage === 2 && (
                <div className="space-y-8 animate-in slide-in-from-right duration-500">
                  <FormSection title="Additional Symptoms" icon={Heart} gradient="from-pink-500 to-rose-600">
                    <FormField
                      label="Cloudy/Strong-Smelling Urine"
                      name="cloudyUrine"
                      type="checkbox"
                      value={formData.cloudyUrine}
                      onChange={handleChange}
                      description="Urine that appears cloudy or has unusual odor"
                    />
                    <FormField
                      ...
                      onChange={handleChange}
                      description="Pink, red, or brownish colored urine"
                    />
                    <div className="lg:col-span-2 space-y-6">
                      <FormField
                        label="Pelvic Pain Severity"
                        name="pelvicPain"
                        type="radio"
                        ...
                        description="Pain in lower abdomen or pelvic region"
                      />
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <FormField
                          label="Fever/Chills"
                          ...
                          onChange={handleChange}
                          description="Body temperature above normal or chills"
                        />
                        <FormField
                          ...
                          onChange={handleChange}
                          description="Feeling sick or actually vomiting"
                        />
                      </div>
                    </div>
                  </FormSection>
              ...
              {currentPage === 3 && (
                <div className="space-y-8 animate-in slide-in-from-right duration-500">
                  <FormSection title="Medical History" icon={Shield} gradient="from-rose-500 to-pink-600">
                    <FormField
                      label="History of Recurrent UTIs"
                      name="recurrentUTI"
                      type="checkbox"
                      value={formData.recurrentUTI}
                      onChange={handleChange}
                      description="More than one UTI in the past year"
                    />
                        ...
                        <div className="group">
                          <div className="flex items-start space-x-4 p-4 rounded-2xl border-2 border-gray-100 hover:border-rose-200 transition-all duration-300 hover:bg-gradient-to-r hover:from-rose-50/50 hover:to-pink-50/50">
                            <div className="relative mt-1">
                              ...
                              >
                                {formData.comorbidities.includes("Diabetes") && <CheckCircle className="h-4 w-4 text-white" />}
                              </div>
                            </div>
                          ...
                          <div className="flex items-start space-x-4 p-4 rounded-2xl border-2 border-gray-100 hover:border-rose-200 transition-all duration-300 hover:bg-gradient-to-r hover:from-rose-50/50 hover:to-pink-50/50">
                            <div className="relative mt-1">
                              <input
                                ...
                                name="comorbidities"
                                value="Kidney Issues"
                                checked={formData.comorbidities.includes("Kidney Issues")}
                                onChange={handleChange}
                                className="sr-only"
                              />
                              <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all duration-300 cursor-pointer ${
                                formData.comorbidities.includes("Kidney Issues")
                                  ? 'bg-gradient-to-r from-rose-500 to-pink-600 border-rose-500 scale-110'
                                  : 'border-gray-300 hover:border-rose-400 bg-white'
                              }`}
                                onClick={() => {
                              ...
                              >
                                {formData.comorbidities.includes("Kidney Issues") && <CheckCircle className="h-4 w-4 text-white" />}
                              </div>
                            </div>
                            <label htmlFor="kidneyIssues" className="flex-1 cursor-pointer">
                              <span className="block text-base font-bold text-gray-900 group-hover:text-rose-900 transition-colors">
                                Kidney Issues
                              </span>
                              <p className="text-sm text-gray-600 mt-1 group-hover:text-rose-700 transition-colors">
                                History of kidney stones, kidney disease, or other issues.
                              </p>
                            </label>
                          </div>
                        </div>
                          ...
                          <div className="flex items-start space-x-4 p-4 rounded-2xl border-2 border-gray-100 hover:border-rose-200 transition-all duration-300 hover:bg-gradient-to-r hover:from-rose-50/50 hover:to-pink-50/50">
                            <div className="relative mt-1">
                              <input
                                ...
                                checked={formData.comorbidities.includes("Other Conditions")}
                                onChange={handleChange}
                                className="sr-only"
                              />
                              <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all duration-300 cursor-pointer ${
                                formData.comorbidities.includes("Other Conditions")
                                  ? 'bg-gradient-to-r from-rose-500 to-pink-600 border-rose-500 scale-110'
                                  : 'border-gray-300 hover:border-rose-400 bg-white'
                              }`}
                                onClick={() => {
                              ...
                              >
                                {formData.comorbidities.includes("Other Conditions") && <CheckCircle className="h-4 w-4 text-white" />}
                              </div>
                            </div>
                            <label htmlFor="otherConditions" className="flex-1 cursor-pointer">
                              <span className="block text-base font-bold text-gray-900 group-hover:text-rose-900 transition-colors">
                                Other Conditions
                              </span>
                              <p className="text-sm text-gray-600 mt-1 group-hover:text-rose-700 transition-colors">
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl p-8 shadow-2xl max-w-xl text-center border-2 border-rose-200">
        ...
        <h1 className="text-3xl font-bold text-gray-900 mb-4">
          Welcome to the UTI Risk Assessment
        </h1>
        <p className="text-gray-600 text-lg mb-8">
          This tool helps you evaluate your potential risk of a Urinary Tract Infection based on your symptoms and medical history.
        </p>
        <button
